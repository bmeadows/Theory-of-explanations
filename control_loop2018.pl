...
representation_granularity/1, communication_specificity/1, complexity_detail/1,
reported/1, join_word/1.
:- dynamic use_pov/1, self_described/0.
...
% Replace the following appropriately for other encodings
read_ASP_program_and_translate_to_predicates :-
	true.
read_ASP_output_and_translate_to_predicates :-
	true.
...
% SPECIFICITY or ABSTRACTNESS words
...
% REPRESENTATION GRANULARITY (coarse/fine distinction) words
preamble_specificity_word(Preamble, FollowWordNetLinks, Ret, Spec) :-
...
print_nonrefined_fluent_outcomes(T, TFinal) :-
	...
	% (TODO for now, find all fluents, regardless of representation_granularity)
